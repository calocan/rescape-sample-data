...
export const locationKey = 'coordinates';
  // The column to use for the node name
export const nodeNameKey ='siteName';
...
const resources = [
  {
    ...
    nodes: createNodes([
      ...
      'Residential Buildings (all typologie);Everywhere in Brussels;NA;Sink;120,000',
...
const groupToNodesAndLinks = (accumulatedGraph, group) => {
  ...
  const nodesByStages = R.addIndex(R.reduce)(
    (accum, node, i) => {
      const {location, isGeneralized} = resolveLocation(R.prop(locationKey, node), i);
      return R.mergeWith(
        (l, r) => R.concat(l, r),
        accum,
        {
